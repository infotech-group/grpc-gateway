...
func Register{{$svc.GetName}}HandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption, authHandler auth.AuthHandler) (err error) {
	...
	return Register{{$svc.GetName}}Handler(ctx, mux, conn, authHandler)
}

// Register{{$svc.GetName}}Handler registers the http handlers for service {{$svc.GetName}} to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func Register{{$svc.GetName}}Handler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn, authHandler auth.AuthHandler) error {
	return Register{{$svc.GetName}}HandlerClient(ctx, mux, New{{$svc.GetName}}Client(conn), authHandler)
}
...
func Register{{$svc.GetName}}HandlerClient(ctx context.Context, mux *runtime.ServeMux, client {{$svc.GetName}}Client, authHandler auth.AuthHandler) error {
	{{range $m := $svc.Methods}}
	{{range $b := $m.Bindings}}
